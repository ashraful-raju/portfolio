<x-app-layout>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <button type="button" x-data=""
                                                x-on:click.prevent="$dispatch('open-modal', 'edit-education-{{ $education->id }}')"
                                                class="text-cyan-500">Edit</button>
                                            <span class="mx-1">|</span>
                                            <form onsubmit="return confirm('Are you sure?')"
                                                action="{{ route('services.destroy', $education->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
    ...
    <x-modal name="add-education" :show="$errors->isNotEmpty()" focusable>
